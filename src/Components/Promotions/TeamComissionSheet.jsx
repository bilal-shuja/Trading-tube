...
const TeamComissionSheet = () => {
    const [promotionSheet, setPromotionSheet] = useState([]);
    const [promotionSheetTemArr, setpromotionSheetTempArr] = useState([]);


    const[teamComissionSheet , setTeamComissionSheet] = useState([]);
    const[teamComissionTempArr , setTeamComissionTemp] = useState([]);
    ...
    const TeamComissionSheetIdentifier = "TeamComissionSheetIdentifier";
    ...
    function gettingAllTeamComission() {
      axios.post(`${process.env.REACT_APP_BASE_URL}fetchallpromotion`)
        .then((res) => {
            setTeamComissionSheet(res.data.Record);
            setTeamComissionTemp(res.data.Record);
        })
        .catch((error) => {
          return error;
        });
    ...
    function TeamComissionSheetFun({ items, index }) {
      ...
      return (
        <>
          <tr key={index + 1} style={{ color: colorScheme.card_txt_color }}>
            ...
            {items.percentage == "10" ? (
              <td style={{ color: "#64dd17" }}>{items.percentage}</td>
            ) : (
              <td style={{ color: "#ff1744" }}>{items.percentage}</td>
            )}
                ...
                <div className="col-12">
                  ...
                  >
                    ...
                    <div className="card-body table-responsive p-2">
                      <div className="row">
                        <Filter
                          TeamComissionData={teamComissionSheet}
                          DateFilter={gettingDate}
                          PhoneFilter={gettingPhone}
                          UsernameFilter={gettingUsername}
                          TeamComissionSheetIdentifier={TeamComissionSheetIdentifier}
                        />
                      </div>
...
export default TeamComissionSheet